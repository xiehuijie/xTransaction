...
class TransactionDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<TransactionEntity>> getTransactionsByType(
    ...
    TransactionType type,
      ...
      (select(transactions)
            ..where((t) =>
                t.ledgerId.equals(ledgerId) & t.type.equalsValue(type))
            ..orderBy([(t) => OrderingTerm.desc(t.timestamp)]))
          .get();
  ...
  Future<List<TransactionAmountDetailEntity>> getAmountDetails(
          int transactionId) =>
      (select(transactionAmountDetail)
  ...
  /// 删除交易的所有金额明细
  Future<int> deleteAmountDetailsByTransactionId(int transactionId) =>
      (delete(transactionAmountDetail)
            ..where((t) => t.transactionId.equals(transactionId)))
          .go();
  ...
  Future<List<TransactionCategoryDetailEntity>> getCategoryDetails(
          int transactionId) =>
      (select(transactionCategoryDetail)
            ..where((t) => t.transactionId.equals(transactionId)))
          .get();
  ...
  Future<int> deleteCategoryDetailsByTransactionId(int transactionId) =>
      (delete(transactionCategoryDetail)
            ..where((t) => t.transactionId.equals(transactionId)))
          .go();
  ...
  Future<TransactionInstallmentDetailEntity?> getInstallmentDetail(
          int transactionId) =>
      (select(transactionInstallmentDetail)
            ..where((t) => t.transactionId.equals(transactionId)))
          .getSingleOrNull();
  ...
  /// 删除交易的所有减免信息
  Future<int> deleteReducesByTransactionId(int transactionId) =>
      (delete(transactionReduce)
            ..where((t) => t.transactionId.equals(transactionId)))
          .go();
  ...
  /// 删除退款信息
  Future<int> deleteRefund(int id) =>
      (delete(transactionRefund)
            ..where((t) => t.transactionRefundId.equals(id)))
          .go();
  ...
  Future<List<int>> getProjectIdsByTransactionId(int transactionId) async {
    final relations = await (select(relationProjectTransaction)
          ..where((t) => t.transactionId.equals(transactionId)))
        .get();
