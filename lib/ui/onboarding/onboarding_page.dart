...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  Future<void> _completeSetup() async {
    ...
    try {
      ...
      final ledgerService = await ref.read(ledgerServiceProvider.future);
      ...
      for (var i = 0; i < state.ledgers.length; i++) {
        ...
        // 创建账本元数据
        final ledgerMeta = await ledgerService.createLedger(
        ...
        if (ledgerConfig.isDefault) {
          defaultLedgerId = ledgerMeta.id;
        }

        // 打开账本数据库
        final db = dbManager.getDatabase(ledgerMeta.id);
        ...
        for (var j = 0; j < state.accounts.length; j++) {
          final account = state.accounts[j];
          
          // 检查账户是否属于此账本
          final includeAccount = ledgerConfig.autoAccount ||
              ledgerConfig.selectedAccountIds.contains(j.toString());
          ...
          accountIdMap[j] = accountId;

          // 保存账户初始余额作为元数据
          if (account.initialBalance != 0) {
            await db.accountDao.upsertAccountMeta(
          ...
          if (account.type == AccountType.credit && account.creditLimit != null) {
            await db.accountDao.insertCreditAccount(
          ...
          if (account.type == AccountType.prepaid && account.enableBonus) {
            // 创建一个关联的赠送金账户
            final bonusAccountId = await db.accountDao.insertAccount(
              AccountCompanion.insert(
            ...
            // 创建预付账户关联
            final bonusMode = switch (account.bonusDeductMode) {
              'first' => AccountBonusMode.first,
              'last' => AccountBonusMode.last,
              _ => AccountBonusMode.same,
            };
            await db.accountDao.insertPrepaidAccount(
        ...
        Future<void> saveCategories(
          ...
          for (var k = 0; k < categories.length; k++) {
            final category = categories[k];
            
            // 检查分类是否属于此账本
            final categoryKey = '${keyPrefix}_$k';
            final includeCategory = ledgerConfig.autoCategory ||
                ledgerConfig.selectedCategoryIds.contains(categoryKey);
            
            if (!includeCategory && parentId == null) continue;
            
            final categoryId = await db.categoryDao.insertCategory(
      ...
      if (state.ledgers.isEmpty) {
        final defaultLedger = await ledgerService.createLedger(
          name: '默认账本',
          currencyCode: 'CNY',
          description: '我的第一个账本',
        );
      ...
      if (defaultLedgerId != null) {
        await ledgerService.setCurrentLedgerId(defaultLedgerId);
      }
